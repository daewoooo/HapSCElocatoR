...
findSegmentsGAM <- function(data=NULL) {
  ...
  segments.df <- data.frame(chromosome=segments.df$seqnames, start=segments.df$start, end=segments.df$end, strand=segments.df$direction, numReads=segments.df$numReads)
